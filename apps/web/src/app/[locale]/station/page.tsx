...
import type { DechargeStationListeDTO, ScanResultDTO } from "@navex/contracts";
import { ApiError, listerDechargesStation, scannerStation } from "@/lib/api-client";
import { formaterDate, messageErreur } from "@/lib/ui";
import { AppHeader } from "@/components/app-header";
import { RequireRole } from "@/components/require-role";
import { Bouton, classesBouton } from "@/components/bouton";

export default function PageStation() {
  const t = useTranslations();
  const locale = useLocale();
  const [decharges, setDecharges] = useState<DechargeStationListeDTO[] | null>(null);
  ...
  const charger = useCallback(() => {
    listerDechargesStation().then(setDecharges).catch((e) => setErreur(messageErreur(t, e)));
  }, [t]);
  ...
  async function traiterScan() {
    ...
    try {
      const resultat = await scannerStation(jeton);
  ...
  const nbEnTransit = decharges?.length ?? 0;
  ...
  return (
    ...
    <div className="min-h-dvh">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 space-y-6">
        <h1 className="text-2xl font-extrabold text-navex-ink">{t("station.titre")}</h1>
        ...
        <section className="rounded-3xl border-2 border-navex-red/40 bg-navex-ink/95 p-6 shadow-glass-lg backdrop-blur-sm">
          ...
          <div className="flex flex-wrap gap-2">
            <input ref={champScan} data-tour="station-scan-input" value={token} onChange={(e) => setToken(e.target.value)} onKeyDown={(e) => e.key === "Enter" && traiterScan()}
              aria-label={t("station.scan_label")} placeholder={t("station.token_placeholder")} dir="ltr" autoComplete="off"
          ...
          <p className="mt-2 text-xs text-neutral-400">{t("station.scan_aide_douchette")}</p>
          ...
          {dernierScan?.kind === "ok" && (
            <div className="mt-4 space-y-2 rounded-2xl bg-navex-red-soft/90 p-4 ring-1 ring-navex-red/20 backdrop-blur-sm">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-navex-ink">
                ...
                <Link href={`/station/decharges/${dernierScan.resultat.decharge_id}`} className="font-semibold text-navex-red underline hover:text-navex-red-dark">{t("station.traiter_decharge")}</Link>
              </div>
            </div>
          )}
          {dernierScan?.kind === "erreur" && (
            <div className="mt-4 rounded-2xl bg-navex-red/10 p-4 ring-1 ring-navex-red/20 backdrop-blur-sm">
              ...
              {dernierScan.dechargeId && (
                <Link href={`/station/decharges/${dernierScan.dechargeId}`} className="mt-1 inline-block text-xs font-semibold text-navex-red underline hover:text-navex-red-dark">{t("station.traiter_decharge")}</Link>
              )}
            </div>
          )}
        </section>

        <section className="card-glass rounded-3xl p-6 ring-glow animate-slide-up">
          <p className="text-xs font-medium uppercase tracking-wider text-neutral-400">{t("station.decharges_en_transit")}</p>
          <p className="mt-2 text-4xl font-extrabold text-navex-red" dir="ltr">{nbEnTransit}</p>
        </section>

        <section className="card-glass-solid rounded-3xl">
          <div className="border-b border-neutral-100/60 px-6 py-4">
            <h2 className="text-sm font-semibold text-navex-ink">{t("station.liste_titre")}</h2>
          </div>
          {!decharges && !erreur && <p className="px-6 py-10 text-center text-sm text-neutral-500">{t("commun.chargement")}</p>}
          {(decharges?.length ?? 0) === 0 && decharges && <p className="px-6 py-10 text-center text-sm text-neutral-500">{t("station.liste_vide")}</p>}
            ...
            {(decharges ?? []).map((d) => (
              <li key={d.id} className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 transition-colors hover:bg-white/40">
                <div className="space-y-0.5">
                  <Link href={`/station/decharges/${d.id}`} className="font-semibold text-navex-ink hover:underline" dir="ltr">{d.numero_decharge}</Link>
                  <p className="text-xs text-neutral-400" dir="ltr">{d.demande.reference} · {d.expediteur_nom} · {d.nb_produits} {t("file_attente.produits_approuves").toLowerCase()} · {formaterDate(d.date_generation, locale)}</p>
                </div>
              </li>
            ))}
          </ul>
